<template>
  <div class="main">
    <div class="content">
      <div class="list-part">
        <div class="shop-info">
          <img :src="'http://localhost:8081/img/'+useStore().shop.logo" class="shopkeeper-avatar">
          <div class="shop-name">
            {{useStore().shop.name}}
          </div>
        </div>
      ...
      <div class="block" v-if="data.chatMessage.length===0">
        <img src="http://localhost:8081/img/chat.dec6e35a.png">
      </div>
      <div class="interface" v-if="data.chatMessage.length!==0">
            ...
            <div class="message-list" ref="messageList">
              <div class="message-li" v-for="(item,index) in data.chatMessage" :key="index">
                ...
                <div class="message-content-right" v-if="item.sendId!==selectedIndex.id">
                  ...
                  <img :src="'http://localhost:8081/img/'+useStore().shop.logo" class="send-avatar">
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="order-part">
          ...
          <div class="no-order" v-if="data.contacts.length===0">暂无订单信息</div>
          <div class="order-list" v-if="data.contacts.length!==0">
            <div class="order-li" v-for="item in data.data">
              <div class="order-id">
                <span>订单号:{{124123131231231231}}</span>
              </div>
              <div class="order-item">
                <img src="http://localhost:8081/img/b3765de87dd31f20bdcd15cc628fecb2.jpg" class="goods-img">
...
import {addChatMessage, getChatMessage, getChatMessageShop, getChatter} from "@/api/chat";
...
const selectItem = (user) => {
  if(selectedIndex.value!==user){
    selectedIndex.value = user;
    const userId = user.id
    const shopId = route.params.shopId;
    getChatMessageShop({userId,shopId}).then(res=>{
      if (res.data.flag){
        data.chatMessage = res.data.data;
...
const getContacts = (shopId) => {
  getChatter({shopId}).then(res=>{
    if(res.data.flag){
...
const initWebSocket = () => {
  ...
  const websock = new WebSocket("ws://localhost:8081/websocket/"+useStore().shop.shopId);
...
const websocketonmessage = (e) => {
  ...
  if (date.cmd === "newChat") {
    ...
    console.log("新的聊天")
    console.log(date)
    getContacts(route.params.shopId)
    ...
    if(selectedIndex.value.id===newChatMessage.sendId){
      ...
      data.contacts.forEach(item=>{
        if (item.id === newChatMessage.sendId){
          item.content = newChatMessage.content
