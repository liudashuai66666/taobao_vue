...
import {addGoods, selectGoodsById, updGoods} from "@/api/goods";
...
const pdSkuKey = () => {
  for (const obj of form.value.sku){
...
const submitForm = () => {
  ...
  ruleFormRef.value.validate((valid) => {
    if (valid) {
      /*编辑商品属性*/
      updGoods(form.value).then(ref=>{
        if (ref.data.flag){
          ElMessage.success(ref.data.data);
          //将购物车中编辑过的商品全部删除
          cartStore().deleteCartByGoodsId(route.query.id);
